...
const requestAsync = async (...options) => new Promise( (success, failure) => {
...
const parseDate = (date) => {
  const [all, day, month, year] = date.match(/^([0-9]{1,2}\.)\s+(.*)\s+([0-9]+)$/)
...
module.exports = async ( url ) => {
  ...
  const { locations, motives, factums, contexts } = parseClassNames( article.attr('class') )

  const description = article.find('.entry-content p').first().text()
